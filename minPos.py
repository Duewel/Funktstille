...
def find_nearest_points_with_time(data, allLat, allLong, angles):
    ...
    for lat, lon, time, angle, future_angle in zip(data["locationLatitude(WGS84)"], data["locationLongitude(WGS84)"], data["time"], angles, data["future_angle"]):
        min_distance = float('inf')
        nearest_corner = None

        # Bestimmen des nächsten Eckpunkts und der Distanz
        ...
        # Überprüfen, ob der aktuelle Punkt der nächste zu einem neuen Eckpunkt ist
        if not nearest_points or nearest_points[-1] != nearest_corner:
            # Füge nur signifikant unterschiedliche Punkte hinzu
            if min_distance < 10:  # Schwellenwert für signifikante Nähe (z.B. 10 Meter)
                nearest_points.append(nearest_corner)
                ...
                nearest_index = (data['time'] - time).abs().idxmin()
